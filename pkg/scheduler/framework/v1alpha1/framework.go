...
// WithBytedInFormerFactory sets byted informer factory for the scheduling framework.
func WithBytedInFormerFactory(bytedinformerFactory bytedinformers.SharedInformerFactory) Option {
	return func(o *frameworkOptions) {
		o.bytedinformerFactory = bytedinformerFactory
	}
}
...
func NewFramework(r Registry, plugins *config.Plugins, args []config.PluginConfig, opts ...Option) (Framework, error) {
	...
	f := &framework{
		...
		bytedinformerFactory:  options.bytedinformerFactory,
...
func (f *framework) BytedInformerFactory() bytedinformers.SharedInformerFactory {
	return f.bytedinformerFactory
}
