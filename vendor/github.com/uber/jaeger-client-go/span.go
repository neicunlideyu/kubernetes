...
type Span struct {
	...
	// flushInterval returns duration of a unfinished span periodic report inside a reporter.
	// Zero value means periodic report is disabled.
...
func (s *Span) setTagNoLocking(key string, value interface{}) {
	...
	for i := range s.tags {
		if s.tags[i].key == key {
			s.tags[i] = Tag{key: key, value: value}
			updated = true
		}
	}
	if !updated {
...
func (s *Span) isPeriodicReportEnabled() bool {
	return s.flushInterval > 0
}
