...
const (
	...
	envReporterPeriodicFlush  = "JAEGER_REPORTER_PERIODIC_FLUSH"
...
func reporterConfigFromEnv() (*ReporterConfig, error) {
	...
	if e := os.Getenv(envReporterPeriodicFlush); e != "" {
		if value, err := strconv.ParseBool(e); err == nil {
			rc.PeriodicFlushSupport = value
		} else {
			return nil, errors.Wrapf(err, "cannot parse env var %s=%s", envReporterPeriodicFlush, e)
		}
	}
