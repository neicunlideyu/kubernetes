...
// CSIStoragePool captures information about a storage pool
// CSIStoragePool objects are non-namespaced.
type CSIStoragePool struct {
	...
	// spec is the specification of CSINodeInfo
	Spec CSIStoragePoolSpec `json:"spec"`

	// status is the current status of CSINodeInfo
	Status CSIStoragePoolStatus `json:"status"`
}
...
type CSIStoragePoolSpec struct {
	...
	// +optional
	// Do not need now, we can re-add this later if needed
	...
	//
	// +optional
	// Do not need now, for local storage, only one node can be set here, and its name is set for the CSIStoragePool's name
	// We can re-add this later if needed
	//Nodes []string
}
...
type CSIStoragePoolStatus struct {
	...
	Classes []CSIStorageByClass `json:"classes,omitempty"`
}
...
type CSIStorageByClass struct {
	...
	/*// Capacity is the size of the Storage Pool with specific Storage Class name
	// key(ResourceName) is the vg name for local storage
	// +optional
	Capacity *v1.ResourceList `json:"capacity,omitempty"`*/

	// CapacityLeftForLocalVolume is the capacity left for creating local volumes
	// key(ResourceName) is the vg name for local storage
	// +optional
	CapacityLeftForLocalVolume *v1.ResourceList `json:"capacityLeftForLocalVolume,omitempty"`
}
...
type CSIStoragePoolList struct {
	...
	// items is the list of CSIStoragePool
	Items []CSIStoragePool `json:"items"`
}
