...
func (t *Tracer) startSpanInternal(
	...
	sp.flushInterval = flushInterval
	...
	if sp.isPeriodicReportEnabled() {
		// If a span should be flushed even unfinished, we can report it right now
		// and later flush just update its duration.
		sp.tracer.reportSpan(sp)
	}
	return sp
}
